	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} Nao volte mais😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD MAFIA BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Yowai? Este é um recurso especial para o Yowai❌',
					ownerB: '[❗] Yowai? Este é um recurso especial para o Yowai ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Yowai. Registre-se para fazer amizade com o bot Yowai por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Yowai|17\n\n──「 Máfiabot」──`,
			...
			const ownerNumber = ["5591984406769@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5591984406769@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5591984406769@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5591984406769@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5591984406769@s.whatsapp.net","559199194727@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		reply(`tchau racista fdp ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		setTimeout( () => {
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("macacopreto")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('cara, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("cadê bot")){
			client.updatePresence(from, Presence.composing)
		...
		if (messagesC.includes("bot")){
			...
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/áudio do dani.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT YOWAI ✨* 」\n*│+ wa.me/5591984406769*╰──────*「 *YOWAI* 」*────*\n\n*_SE QUER SER ADMIN DO MÁFIABOT_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* YOWAI\\n*WPP:* wa.me/+5591984406769\Espero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
